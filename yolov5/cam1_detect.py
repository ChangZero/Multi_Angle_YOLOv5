...
def h_distance(xyxy, h_location):
    ...
    for h in h_location:
        distance = math.sqrt(math.pow(
            x_center - h_location[h][0], 2) + math.pow(y_center - h_location[h][1], 2))
        distance_dict[h] = distance
...
# Area based Rule
def compare_intersection(w_xyxy, h_xyxy):
    h_area = (h_xyxy[2] - h_xyxy[0]) * (h_xyxy[1] - h_xyxy[3])
    if h_area==0:
        return 0
    ...
    return intersection_area / h_area
...
def run(
    ...
    file_path = './hole_json_file/cam1_h_status.json'
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        if cls == 2:
                            flag = 1
                            w_to_h = h_distance(xyxy, h_location)
                            detbox["wrench"] = xyxy

                            label = None if hide_labels else (
                                names[cls] if hide_conf else f'{names[cls]} {conf:.2f}')
                            # label = "wrench"
                            annotator.box_label(
                                xyxy, label, color=colors(cls, True))
                        elif (cls == 0) | (cls == 1):
                        # if any(c in label for c in ('open','closed')): # open/closed/complete일 경우에만 ID할당
                            h_info = h_distance(xyxy, h_location) #ID 생성
                            
                            detbox[h_info[0]] = xyxy
                            if h_status[h_info[0]] == 4:
                                cls = 4
                            else:
                                h_status[h_info[0]] = cls
                            label = None if hide_labels else (
                                names[cls] if hide_conf else f'{names[cls]} {conf:.2f}')
                        
                            label = f'{h_info[0]}: {label}'
                            annotator.box_label(
                                xyxy, label, color=colors(cls, True)) # opencv 활용하여 이미지에 box출력
                        else:
                            label = None if hide_labels else (
                                names[cls] if hide_conf else f'{names[cls]} {conf:.2f}')
                            annotator.box_label(
                    ...
                    if save_crop:
                        save_one_box(xyxy, imc, file=save_dir / 'crops' /
                                     names[cls] / f'{p.stem}.jpg', BGR=True)
